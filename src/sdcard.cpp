...
{
  status->requestLogStopWrite = false;
...
void writeSDLogEntry(statuses* status, uint32_t timeMs)
{
    ...
    double unk = 0.00;
        ...
        rb.printf("%4.1f\t", status->idleLoad * 1.0);    // Duty Cycle			4.6	 (idle)
        ...
        rb.printf("%4.1f\t", status->fuelLoad * 1.0);    // FuelLoad				51.0	
        rb.printf("%4.1f\t", status->ignLoad * 1.0);    // IgnitionLoad			51.0	
        ...
        rb.printf("%4.1f\t", status->vss * 1.0);    // Vehicle Speed		0.00	
        ...
        if (status->requestLogStopWrite) {
          endSDLogging(status);
        }
        else if (status->sdCardblockCount % 512 == 0)
        {
            ...
            */
            ...
            logFile.sync();
            bool retOpen = false;
            retOpen = logFile.open(path, O_WRONLY | O_APPEND | O_AT_END);
            if (retOpen == false) {
              status->sdCardStatus = SD_STATUS_ERROR_WRITE_FAIL;
            }
            else {
              status->sdCardStatus = SD_STATUS_ACTIVE;
            } 
